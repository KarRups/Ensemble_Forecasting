...
def run_hyperparameter_test():
    ...
    learning_rates = [1e-3]  #[1e-2, 1e-3, 1e-4]
    bidirectional_options = [False] #[False, True]
    hidden_sizes = [
        # (16, 16, 8) #,    # (hindcast_hidden, forecast_hidden, handoff_hidden)
        # (32, 32, 16) #,
        (256, 256, 64),
        (256, 256, 128)
    ...
    forecast_input_size = 15 #Non Binary
    hindcast_output_size = 8
    forecast_output_size = 12
    
    batch_accumulation = 2000
    
    # Create directory for models if it doesn't exist
    model_dir = '/home/mokr/Loss_Functions_Paper/Models/NonBinary/hyperparameter_test'
            ...
            for epochs in num_epochs:
                for hidden_size in hidden_sizes:
                    ...
                    train_loss = ML_functions.train_model_CMAL(
                        ...
                        epochs=10, train_mode=True,
                        batch_accumulation=batch_accumulation, device=device, scheduler = scheduler, use_observations = False
                    )
                    ...
                    # Save model
                    model_path = os.path.join(model_dir, f"NonBinary_{config_desc}_{timestamp}_5Epochs.pth")
                    ...
                    val_metrics = ML_functions.train_model_CMAL(
                        ...
                        batch_accumulation= batch_accumulation, device=device, scheduler = None, use_observations = False
                    )
                    ...
                    model_path = os.path.join(model_dir, f"NonBinary_{config_desc}_{timestamp}_15Epochs.pth")
                    torch.save(model, model_path)
                    print(f"Model saved to {model_path}")
                    ...
                    train_loss = ML_functions.train_model_CMAL(
                        model, Training_Dataloader, optimizer, criterion,
                        epochs= 5, train_mode=True,
                        batch_accumulation=batch_accumulation, device=device, scheduler = scheduler, use_observations = False
                    )

                    model_path = os.path.join(model_dir, f"NonBinary_{config_desc}_{timestamp}_30Epochs.pth")
                    torch.save(model, model_path)
                    print(f"Model saved to {model_path}")
